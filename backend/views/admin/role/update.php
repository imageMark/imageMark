            ...
            <div class="box-body">
                ...
                foreach($tops as $k=> $top){?>
                    <div class="col-md-12 bg-gray" >
                        <label><input type="checkbox" name="Permission[]" value="<?=$top['id']?>" <?php  if(in_array($top['id'],$Permission)):?> checked  <?php endif;?> class="top" data-key="<?=$k?>" ><?php echo $top['name']; ?></label>
                    </div>
                    ...
                    <?php if(isset($top['child'])){foreach($top["child"] as $key=> $one){?>
                        ...
                        <div class="col-md-11">
                            <label style="width: 100px;">
                                <input type="checkbox" name="Permission[]" <?php  if(in_array($one['id'],$Permission)):?> checked  <?php endif;?> value="<?=$one["id"]?>" class="one" data-key="<?=$k?>"  id="limit-<?=$k?>-<?=$key?>" ><?php echo $one['name']; ?>
                            </label>
                            <?php if(isset($one['child'])){foreach($one["child"] as $two){?>
                                <label style="font-weight: inherit"> <input type="checkbox" name="Permission[]" <?php  if(in_array($two['id'],$Permission)):?> checked  <?php endif;?> value="<?=$two['id']?>" class="limit-<?=$k?>-<?=$key?>" data-tag="two" data-key="<?=$k?>"  style="margin-left: 20px;"><?php echo $two['name']; ?></label>
                            <?php }
                            } ?>
                        </div>
                        <div class="help-block"></div>
